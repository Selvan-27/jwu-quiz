...
@section('content')
    <h1 class="h3 mb-4 text-gray-800">Add Question to: {{ $quiz->title }}</h1>
                ...
                <div class="input-group mb-3">
                    <div class="input-group-prepend">
                        <div class="input-group-text">
                            <input type="radio" name="correct_option" value="{{ $i }}" {{ old('correct_option') == $i ? 'checked' : ($i == 0 ? 'checked' : '') }} required>
                        </div>
                    </div>
                    <input type="text" name="options[]" class="form-control" placeholder="Option {{ $i + 1 }}" value="{{ old('options.'.$i) }}" required>
                </div>
                @endfor
